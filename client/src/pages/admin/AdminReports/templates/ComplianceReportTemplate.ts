...
export const generateComplianceReport = async (
  ...
  if (reportData.standards) {
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(COLORS.text.r, COLORS.text.g, COLORS.text.b);
    doc.text('📜 Regulatory Standards', SPACING.page.left, yPos);
    yPos += 2;
    ...
    // Standards table
    const standardsData = [
      ['💧 Turbidity', standards.turbidity || '< 5 NTU', 'WHO Guidelines'],
      ['⚗️ TDS', standards.tds || '< 500 ppm', 'WHO Guidelines'],
      ['🔬 pH', standards.ph || '6.5 - 8.5', 'WHO Guidelines'],
    ];
  ...
  if (reportData.summary) {
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(COLORS.text.r, COLORS.text.g, COLORS.text.b);
    doc.text('📊 Compliance Summary', SPACING.page.left, yPos);
    yPos += 2;
    doc.setDrawColor(COLORS.secondary.r, COLORS.secondary.g, COLORS.secondary.b);
    doc.setLineWidth(0.8);
    doc.line(SPACING.page.left, yPos, SPACING.page.left + 50, yPos);
    ...
    doc.setFont('helvetica', FONTS.body.style);
    doc.text(`• Total Devices: ${summary.totalDevices || 'N/A'}`, SPACING.page.left + 5, yPos + 15);
    doc.text(`• Compliant Devices: ${summary.compliantDevices || 'N/A'}`, SPACING.page.left + 5, yPos + 21);
    doc.text(`• Compliance Rate: ${complianceRateStr}`, SPACING.page.left + 5, yPos + 27);
  ...
  if (reportData.devices && reportData.devices.length > 0) {
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(COLORS.text.r, COLORS.text.g, COLORS.text.b);
    doc.text('🔍 Device Compliance Details', SPACING.page.left, yPos);
    yPos += 2;
    doc.setDrawColor(COLORS.secondary.r, COLORS.secondary.g, COLORS.secondary.b);
    doc.setLineWidth(0.8);
    ...
    for (const deviceCompliance of reportData.devices) {
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(COLORS.text.r, COLORS.text.g, COLORS.text.b);
      doc.text(`📟 ${deviceCompliance.deviceName || deviceCompliance.deviceId}`, SPACING.page.left + 3, yPos + 6.5);
      ...
      if (deviceCompliance.complianceStatus) {
        ...
        const complianceData = deviceCompliance.complianceStatus.map((status: any) => {
          const paramEmoji = status.parameter === 'Turbidity' ? '💧' : 
                           status.parameter === 'TDS' ? '⚗️' : 
                           status.parameter === 'pH' ? '🔬' : '📊';
          return [
